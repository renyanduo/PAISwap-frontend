...
const LP_STAKING = new Contract(CONFIG["lpContractAddress"], LP_ABI, getDefaultProvider(CONFIG["network"]));

const UNISWAP_STAKING = new Contract(CONFIG["uniswapContractAddress"], UNI_ABI.abi, getDefaultProvider(CONFIG["network"]));
...
/**
 * 提取LP
 * @param {*} 提取Lp的数量，LP的精度为18，数值按照精度换算好 {value}
 * @returns 
 */
export function withdraw(value) {
    return new Promise(async (resolve, reject) => {
        try {
            const LP_STAKING = new Contract(CONFIG["lpContractAddress"], LP_ABI, new Web3Provider(window.web3.currentProvider).getSigner());
            const list = await LP_STAKING.withdraw(parseEther(value))
            resolve(list)
        } catch (error) {
            reject(error)
        }
