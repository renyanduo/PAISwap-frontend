...
import { getStaking, getBalance, withdraw, deposit, Approve, getAllowance, getTotalSupply, getPendingReward, getRedemption, getApy } from '../../util/pool/Lp';
...
function Lp(props) {
    ...
    const getYield = () => {
        getApy().then(async (e) => {
            ...
            let apyValve = toPercent((((paiChainBlockToDay * (Number(e.totalReward)) * pnft_pi) * Number(piUsdt)) / (Number(totalStaking) * Number(piUsdt))) * 1)
            setApyValve(apyValve)
        }).catch(e => {
            console.log(e);
        });
    }
    ...
    const confirmWithdraw = () => {
        console.log(unInputValue);
        withdraw(unInputValue).then(e => {
            openNotificationWithIcon('info', 'Submitted on the chain, please wait for confirmation on the chain.')
            e.wait().then(w => {
                openNotificationWithIcon('success', 'Successful transaction!')
                userAddress && initialize(userAddress)
